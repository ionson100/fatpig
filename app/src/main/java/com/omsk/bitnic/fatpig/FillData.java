...
//        panel4.setBackgroundColor(Settings.getSettings().colorPanel4);
        ...
        long idd = today.getTime().getTime();
        ...
        }
        {
            ...
            Map<Long, List<GeoData>> dataMap = Linq.toStream(geoDatas).groupBy(new Function<GeoData,Long>() {
                @Override
                public Long foo(GeoData t) {
                    return t.trackName;
                }
            });

            double cal = 0;
            for (Long ss : dataMap.keySet()) {
